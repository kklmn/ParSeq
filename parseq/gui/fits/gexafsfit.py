...
class EXAFSFitWidget(gbf.FitWidget):
    ...
    Consider the supplied project file ``saved/Cu.pspj``. In the BFT tab,
    click the "EXAFS fit" button below the plot, select the first data item,
    and hide the others (use the eye icon in the data tree header). Then
    press "Start fitting" to generate the fit curve.

    The first tab of the fit widget corresponds to the first coordination
    shell (in this example, a single shell is used). The last tab contains
    the factor :math:`S_0^2` along with any user-defined metavariables.

    Hover the mouse cursor over the table headers to view descriptions of
    ...
    The last two columns display fitting errors computed in two ways:
    (a) without considering pair correlations, and (b) including all pair
    correlations. The latter should generally be preferred, but it may fail
    if the :math:`\chi^2` function does not reach a true minimum.

    In such cases, the table of pair correlation coefficients can provide
    useful diagnostic information. It can be displayed by clicking on the
    headers of the error columns. Problematic variables identified in this
    way should be constrained or fixed to improve the stability of the fit.
    ...
    Each coordination shell requires a FEFF file containing scattering
    amplitudes and phases (``feffNNNN.dat``), generated by the FEFF program
    (specifically the *ff2chi* module with PRINT level set to 3). Use the
    |icoAP| button to open the FEFF file selection dialog. This dialog scans
    the current directory for ``feffNNNN.dat`` files and highlights in green
    those corresponding to single-scattering paths.

    The preview panel displays the header of the selected file, including a
    summary shown in bold green text. Once a file is confirmed, its *deg*
    (degeneracy) and *reff* (effective radius) values are used to initialize
    the shell parameters *n* and *r*.

    .. tip::

       Multiple ``feffNNNN.dat`` files can be selected simultaneously, which
       results in the creation of multiple coordination shells.

    .. note::

       Multiple-scattering paths are handled in the same way as
       single-scattering ones. Appropriate tie expressions should be applied
       to coordination numbers and, if necessary, to interatomic distances.

    The fit uses four parameters per shell: ``r1``, ``n1``, ``s1``, ``e1``,
    ``r2``, and so on. The global amplitude factor :math:`S_0^2` is represented
    by the variable ``s0``. These variable names can be referenced in tie
    expressions. Examples of valid expressions include ``"fixed"``,
    ``"=n1*0.5"``, and ``">4.4-r1"`` (e.g., for the ``r2`` parameter,
    enforcing :math:`r_1 + r_2 > 4.4 \,\text{\AA}`).

    .. note::

       The :math:`E_0` shift parameters (``e``) for different shells should
       not differ significantly. The acceptable variation depends on how the
       atomic potentials were calculated. If they were obtained
       self-consistently, the shifts should be identical. In any case,
       shells corresponding to the same chemical element should share the
       same energy shift.
    ...
    Tie expressions may refer to fit parameters from other data items.
    Consider the project file ``saved/CeRu2.pspj``, where the same
    interatomic distance is probed at two different absorption edges.

    The tie expression ``"=fit['CE0074'].r1"`` assigns the parameter
    ``r2`` to be equal to ``r1`` of the data item with alias
    ``'CE0074'``. In this way, two otherwise independent fits are
    coupled into a single, larger fitting problem.

    The resulting correlation matrix can be examined to identify the
    relationships between the involved fitting parameters.
    """
